...
class AStar {
  ...
  compute () {
    ...
    while (!this.openHeap.isEmpty()) {
      if ((nodesExpanded & 31) === 0) { // check time every 32 nodes
        const now = performance.now()
        if (now - computeStartTime > this.tickTimeout) { // compute time per tick
          return this.makeResult('partial', this.bestNode)
        }
        if (now - this.startTime > this.timeout) { // total compute time
      ...
      this.visitedChunks.add((node.data.x >> 4) * 67108864 + (node.data.z >> 4))
